...
void parse_cmd_params(int _argc, char **_argv, int &_scale, int &_avg_degree,
                      bool &_check, string &_graph_type, bool &_load_from_file, string &_file_name, bool &_convert, string &_convert_name, int &_iterations)
{
...
struct TempEdgeData
{
    int dst_id;
    float weight;
    TempEdgeData(int _dst_id, float _weight) { dst_id = _dst_id; weight = _weight; };
};
...
void convert_edges_list_to_CSR(Graph &_edges_list_graph, GraphCSR &_csr_graph)
{
    ...
    {
        const long long edge_start = _csr_graph.outgoing_ptrs[i]; // получаем положение первого ребра вершины
        const int connections_count = _csr_graph.outgoing_ptrs[i + 1] - _csr_graph.outgoing_ptrs[i]; // получаем число смежных ребер вершины
