...
void streams_vec_add(int size = 1048576)
{
    ...
    for ( int i = 0; i < NbStreams; i++ )
    {
        ...
        SAFE_CALL(cudaMemcpyAsync(&a_d[ Offset ], &a[ Offset ], StreamSize * sizeof(float), cudaMemcpyHostToDevice, Stream[ i ]));
        ...
        SAFE_KERNRL_CALL((vectorAddGPU<<<grid, block>>>(a_d, b_d, c_d, StreamSize, Offset)));
